...
export default {
  ...
  methods: {
    editHandler(row) {
      ...
      if (row.type === 'Radio')
        this.buttonType = ['Radio', 'RadioButton']
      ...
      if (row.cascader) {
        ...
        this.json = {
          options: row.cascader.options
        }
      }
      if (row.options)
        this.textarea = row.options.join('\n')
      this.cardShow = false
    ...
    confirm() {
      ...
      const rules = this.rules.reduce((r, item) => {
        r[item.regExp] = item.errMsg
        return r
      }, {})
      const data = {
        id: this.input,
          ...
          <el-tabs v-model="activeName" class="demo-tabs">
            <el-tab-pane label="Basic settings" name="first">
              <div v-show="cardType">
                ...
                <VueJsonEditor v-if="cardType === 'Cascader'" style="margin-top:20px;" v-model="cascader.options"
                  :expanded-on-start="true" :mode="mode" />

                <el-input v-show="
                  cardType === 'Enumeration' || showType.includes(cardType)
                " v-model="textarea" :rows="5" type="textarea" placeholder="Ex:
  morning
  noon
  evening" />
              </div>
              ...
              <div class="flex flex-col item-start" style="margin-bottom: ;20px;">
                ...
                <div class="wrapper left">
                  <div class="w45" text-left flex flex-col>
                    ...
                    <el-input-number v-show="min" v-model="number.min" :min="0" :max="10" size="small"
                      controls-position="right" />
                  </div>
                  <div class="w45" text-left flex flex-col>
                    <el-checkbox v-model="max" label="最大值" size="large" />
                    <el-input-number v-show="max" v-model="number.max" :min="0" :max="10" size="small"
                      controls-position="right" />
                  </div>
                </div>
              </div>
